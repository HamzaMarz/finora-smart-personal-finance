import path from 'path';
import { defineConfig, loadEnv } from 'vite';
import react from '@vitejs/plugin-react';
import { VitePWA } from 'vite-plugin-pwa';

export default defineConfig(({ mode }) => {
  const env = loadEnv(mode, '.', '');
  return {
    server: {
      port: 3000,
      host: '0.0.0.0',
      proxy: {
        '/auth': 'http://localhost:5000',
        '/user': 'http://localhost:5000',
        '/summary': 'http://localhost:5000',
        '/income': 'http://localhost:5000',
        '/expenses': 'http://localhost:5000',
        '/savings': 'http://localhost:5000',
        '/investments': 'http://localhost:5000',
        '/reports': 'http://localhost:5000',
        '/notifications': 'http://localhost:5000',
        '/ai-insights': 'http://localhost:5000',
        '/backup': 'http://localhost:5000',
        '/api': 'http://localhost:5000',
      }
    },
    plugins: [
      react(),
      VitePWA({
        registerType: 'autoUpdate',
        includeAssets: ['assets/urwallet_logo.png', 'favicon.ico', 'apple-touch-icon.png', 'masked-icon.svg'],
        manifest: {
          name: 'UrWallet - محفظتك الشخصية',
          short_name: 'UrWallet',
          description: 'Smart Personal Finance Management Application',
          theme_color: '#ffffff',
          icons: [
            {
              src: 'assets/urwallet_logo.png',
              sizes: '192x192',
              type: 'image/png'
            },
            {
              src: 'assets/urwallet_logo.png',
              sizes: '512x512',
              type: 'image/png'
            }
          ]
        },
        workbox: {
          globPatterns: ['**/*.{js,css,html,ico,png,svg,woff2}'],
          runtimeCaching: [
            {
              urlPattern: /^https:\/\/fonts\.googleapis\.com\/.*/i,
              handler: 'CacheFirst',
              options: {
                cacheName: 'google-fonts-cache',
                expiration: {
                  maxEntries: 10,
                  maxAgeSeconds: 60 * 60 * 24 * 365 // <== 365 days
                },
                cacheableResponse: {
                  statuses: [0, 200]
                }
              }
            },
            {
              urlPattern: /^https:\/\/fonts\.gstatic\.com\/.*/i,
              handler: 'CacheFirst',
              options: {
                cacheName: 'gstatic-fonts-cache',
                expiration: {
                  maxEntries: 10,
                  maxAgeSeconds: 60 * 60 * 24 * 365 // <== 365 days
                },
                cacheableResponse: {
                  statuses: [0, 200]
                }
              }
            }
          ]
        }
      })
    ],
    define: {
      'process.env.API_KEY': JSON.stringify(env.GEMINI_API_KEY),
      'process.env.GEMINI_API_KEY': JSON.stringify(env.GEMINI_API_KEY)
    },
    resolve: {
      alias: {
        '@': path.resolve(__dirname, '.'),
      }
    }
  };
});
